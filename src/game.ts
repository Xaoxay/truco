...
export function newMatch(
  ...
  return {
    id,
    names: names.map(
      (n, i) => n.trim().slice(0, 24) || ["Nosotros", "Ellos"][i],
    ) as [string, string],
    goal,
    moves: [],
    startedAt: Date.now(),
...
export function scores(match: Match): [number, number] {
  return match.moves.reduce<[number, number]>(
    (sum, move) => {
      sum[move.team] += move.points;
      return sum;
    },
    [0, 0],
  );
}
export function winner(match: Match): Team | null {
  const s = scores(match);
  return s[0] >= match.goal ? 0 : s[1] >= match.goal ? 1 : null;
}
export function addPoints(match: Match, team: Team, points: number): Match {
  ...
  return actual === 0
    ...
    : {
        ...match,
        moves: [...match.moves, { team, points: actual, at: Date.now() }],
      };
}
export function undo(match: Match): Match {
  return { ...match, moves: match.moves.slice(0, -1) };
}
...
function validMatch(value: unknown): value is Match {
  ...
  if (
    ...
    m.names.length !== 2 ||
    !m.names.every(
      (n) => typeof n === "string" && n.length > 0 && n.length <= 24,
    ) ||
    ![15, 30].includes(m.goal) ||
